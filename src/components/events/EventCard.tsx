...
export function EventCard({ event, variant = 'default' }: EventCardProps) {
  ...
  return (
    <Link to={`/community/events/${event.slug}`}>
      <Card className={`group overflow-hidden transition-all hover:shadow-xl ${
        ...
        <div className={`relative overflow-hidden bg-gradient-to-br from-primary/10 to-primary/5 ${
          ...
          {/* Date Badge */}
          <div className="absolute left-4 top-4 flex flex-col items-center rounded-lg bg-background px-3 py-2 shadow-lg">
        ...
        {/* Content */}
        <div className={`flex flex-col p-5 ${isFeatured ? 'lg:w-3/5' : ''}`}>
          <h3 className={`font-bold line-clamp-2 group-hover:text-primary ${
          ...
          <div className="mt-auto pt-4 flex items-center justify-between border-t">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>
                {event.rsvp_count} {event.rsvp_count === 1 ? 'guest' : 'guests'}
