...
export default function MessageMentorModal({ 
  ...
  const [message, setMessage] = useState(`Hi ${mentorName || 'there'}, I'm interested in your mentorship.`);
  ...
  const handleSend = async () => {
    try {
      setSending(true);
      const { conversationId } = await getOrCreateConversation(mentorUserId, message);
      toast({ title: 'Message sent' });
      onClose();
      navigate('/inbox', { state: { openConversationId: conversationId, mentorName } });
    } catch (e: any) {
