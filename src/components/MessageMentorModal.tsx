  ...
  const loadMessages = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('messages')
        .select('*')
        .or(`and(sender_id.eq.${user?.id},recipient_id.eq.${mentorUserId}),and(sender_id.eq.${mentorUserId},recipient_id.eq.${user?.id})`)
  ...
  const sendMessage = async () => {
    ...
    try {
      const { error } = await supabase
        .from('messages')
        .insert({
          listing_id: mentorId, // Using mentor ID as listing ID for compatibility
          sender_id: user.id,
          recipient_id: mentorUserId,
    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-background border rounded-lg shadow-lg w-full max-w-lg max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b">
          <div>
            <h3 className="text-lg font-semibold">Message {mentorName}</h3>
